...
class uav_High_Level_Controller():
    ...
    def euler_angle_controller(self, timer):
        ...
        data = torch.Tensor([[state.pose.pose.position.x, state.pose.pose.position.y, state.pose.pose.position.z],
                             ...
                             Rbw[1],
                             Rbw[2],
                             [trajectory.pose.position.x, trajectory.pose.position.y, trajectory.pose.position.z],
                             [trajectory.twist.linear.x, trajectory.twist.linear.y, trajectory.twist.linear.z],
                             [trajectory.acc.x, trajectory.acc.y, trajectory.acc.z],
                             [trajectory.jerk.x, trajectory.jerk.y, trajectory.jerk.z],
                             [0.0, 0.0, 0.0],
                             Rbw_ref[0],
                             Rbw_ref[1],
                             Rbw_ref[2],
                             [trajectory.yaw, trajectory.yawdot, trajectory.yawddot],
                             [trajectory.uc.x, trajectory.uc.y, trajectory.uc.z]])
        ...
        start.record()
        self.T, self.Rbw_des, w_des = self.ggc.position_controller(data)
        end.record()
...
if __name__ == '__main__':
    try:
        ...
        df.to_csv("~/Documents/gputimes3.csv", index = None, sep = ",")
