...
def euler_from_matrix(R):
    ...
    """
    device = R.device
    theta = torch.asin(-1.0 * R[2][0]).to(device)
    yx_comps = torch.div(torch.tensor([R[2][1], R[2][2], R[1][0], R[0][0]]), torch.cos(theta))
    phi = torch.atan2(yx_comps[0],yx_comps[1]).to(device)
    psi = torch.atan2(yx_comps[2],yx_comps[3]).to(device)
    #phi = torch.atan2(R[2][1] / torch.cos(theta), R[2][2] / torch.cos(theta))
    #psi = torch.atan2(R[1][0] / torch.cos(theta), R[0][0] / torch.cos(theta))

    return phi, theta, psi#torch.tensor([phi, theta, psi])
...
def rotation_matrix_distance(R1, R2):
    ...
    """
    Rd = torch.mm(torch.t(R1), R2)       # get difference rotation matrix
    angle = torch.acos((torch.trace(Rd) -1.0 )/2.0)
...
def vex2(mx): 
    ...
    """
    device = mx.device
    v = torch.tensor([[0.0],[0.0],[0.0]]).to(device)
    v[2][0] = mx[1][0]
    v[1][0] = mx[0][2]
    v[0][0] = mx[2][1]
